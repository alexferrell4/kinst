...
const categoriesEs = [
  'Carrera de Exploración',
  'Autodefensa',
  'Preparación Del Lugar De Trabajo',
  'Aprendizaje Basado En El Trabajo',
  'Consejería Sobre Oportunidades Post Secundarias',
]
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <article className="rounded-2xl border border-border bg-card p-8 shadow-sm">
            ...
            <p className="mt-5 text-base leading-relaxed text-foreground">
              El Instituto de Desarrollo Konnecting the Dots ofrecerá Servicios
              de Transición Pre-Empleo (Pre ETS). Este programa en constante
              evolución capacitará a los estudiantes con prácticas de
              desarrollo, actitudinales y éticas esenciales para fomentar una
              ciudadanía productiva. Los estudiantes deben tener entre 14 y 22
              años. Este programa educativo se estructura en tres fases.
            </p>
            <h2 className="mt-7 font-heading text-lg font-semibold text-foreground">
              Categorías:
            </h2>
            <ul className="mt-4 space-y-3">
              {categoriesEs.map((label, i) => {
                const Icon = categoriesEn[i].icon
                return (
                  <li key={label} className="flex items-center gap-3">
                    <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-accent text-accent-warm-foreground">
                      <Icon className="size-4" />
